...
const applyBuildDebugFlagsToWindow = () => {
  ...
  const error = getBuildBooleanFlag("error");
...
const TASKPANE_SUBTITLE_DESKTOP = "Vejice urejate s Sledi spremembam.";
  ...
  try {
    ...
    if (summary?.status === "deferred") {
      ...
      if (totalFixed === 0 && detected === 0 && apiErrors === 0 && nonCommaSkips === 0) {
        ...
        setStatus(`Kon\u010dano. Popravki: ${totalFixed}.`);
      } else {
        setStatus("Kon\u010dano.");
      }
    }
...
const runClearHighlights = async () => {
  ...
  try {
    ...
    if (summary?.status === "deferred") {
      ...
      publishTaskpaneNotifications(
        ["Namig: Znova za\u017eenite preverjanje in nato pobri\u0161ite ozna\u010dbe."],
        { source: "taskpane", level: "info" }
      );
    }
  ...
  try {
    ...
    if (summary?.status === "applied" || summary?.status === "partial") {
      if (summary?.reason === "suggestion-skipped-unresolvable") {
        setStatus(`Predloga ni bilo mogoce sprejeti, zato je bil preskocen. Preostalo: ${pendingAfter}.`);
      } else {
...
Office.onReady((info) => {
  ...
  const taskpaneSubtitle = document.getElementById("taskpane-subtitle");
