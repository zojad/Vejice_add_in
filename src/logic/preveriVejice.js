...
const readBuildBooleanFlag = (symbolName) => {
  try {
    switch (symbolName) {
      ...
      case "error":
        return typeof __VEJICE_BUILD_ERROR_LOGS__ === "boolean" ? __VEJICE_BUILD_ERROR_LOGS__ : undefined;
      case "onlineVerbose":
...
const BUILD_ERROR_LOGS = readBuildBooleanFlag("error");
...
const ONLINE_DRIFT_LOGS_OVERRIDE =
  typeof window !== "undefined" && typeof window.__VEJICE_ONLINE_DRIFT_LOGS__ === "boolean"
    ...
    : typeof process !== "undefined"
      ...
      : undefined;
const ERROR_LOGS_OVERRIDE =
  typeof window !== "undefined" && typeof window.__VEJICE_ERROR_LOGS__ === "boolean"
    ? window.__VEJICE_ERROR_LOGS__
    : typeof BUILD_ERROR_LOGS === "boolean"
      ? BUILD_ERROR_LOGS
    : typeof process !== "undefined"
      ? parseQuietBoolean(process.env?.VEJICE_ERROR_LOGS)
      : undefined;
const QUIET_LOGS = typeof QUIET_LOGS_OVERRIDE === "boolean" ? QUIET_LOGS_OVERRIDE : true;
const ERROR_LOGS = typeof ERROR_LOGS_OVERRIDE === "boolean" ? ERROR_LOGS_OVERRIDE : true;
...
const LONG_PARAGRAPH_MESSAGE = "je predolg za preverjanje, razdelite ga na več odstavkov in poskusite znova.";
...
const longParagraphNotificationIndexes = new Set();
...
function queueLongParagraphSummaryNotification() {
  ...
  const labels = Array.from(longParagraphNotificationIndexes)
    ...
    .map((index) => index + 1);
  if (!labels.length) {
    longParagraphNotificationIndexes.clear();
    return;
  }
  let message = "";
  if (labels.length === 1) {
    message = `Odstavek ${labels[0]} ${LONG_PARAGRAPH_MESSAGE}`;
  } else if (labels.length === 2) {
    message = `Odstavka ${labels[0]} in ${labels[1]} sta predolga, razdelite ju na več odstavkov in poskusite znova.`;
  } else {
    message = `Nekateri odstavki so predolgi (presegajo ${MAX_PARAGRAPH_CHARS} znakov), razdelite jih na več odstavkov in poskusite znova.`;
  }
  queueScanNotification(message, "warn");
  longParagraphNotificationIndexes.clear();
}
...
function notifyParagraphTooLong(paragraphIndex, length) {
  ...
  if (Number.isFinite(paragraphIndex) && paragraphIndex >= 0) {
    longParagraphNotificationIndexes.add(paragraphIndex);
    return;
  }
  queueScanNotification(`Odstavek ? ${LONG_PARAGRAPH_MESSAGE}`, "warn");
}
...
function notifySentenceTooLong(paragraphIndex, length) {
  ...
  queueScanNotification(LONG_SENTENCE_MESSAGE, "warn");
}
...
async function highlightInsertSuggestion(context, paragraph, suggestion) {
  ...
  const highlightAnchorCandidate = shouldForcePunctuationBoundary
    ...
    if (!range && !shouldPreferQuoteBoundary && !shouldForcePunctuationBoundary) {
      const pairBeforeAnchor = [
        anchor?.boundaryMeta?.beforeToken,
        boundaryBeforeLexicalAnchor,
        anchor.sourceTokenBefore,
        anchor.targetTokenBefore,
      ].find(isLexicalAnchorCandidate);
      const pairAfterAnchor = [
        anchor?.boundaryMeta?.afterToken,
        boundaryAfterLexicalAnchor,
        anchor.sourceTokenAfter,
        anchor.targetTokenAfter,
      ].find(isLexicalAnchorCandidate);
      const pairExpectedBoundarySourceHint = firstFiniteValue([
        plannedBoundaryPos,
        lexicalBoundaryHint,
        anchor?.boundaryMeta?.sourceBoundaryStart,
        anchor?.boundaryMeta?.sourceBoundaryPos,
      ...
      const liveText = typeof paragraph?.text === "string" ? paragraph.text : baseText;
      const sourceTextForPair = typeof baseText === "string" ? baseText : "";
      const expectedBoundaryLiveHint = Number.isFinite(pairExpectedBoundarySourceHint)
        ? sourceTextForPair.length
          ? mapIndexAcrossCanonical(sourceTextForPair, liveText, pairExpectedBoundarySourceHint, {
              allowEnd: true,
            })
          : Math.max(0, Math.min(liveText.length, Math.floor(pairExpectedBoundarySourceHint)))
        : -1;
      const boundaryMeta = anchor?.boundaryMeta || {};
      const hasBoundaryFingerprintHints =
        (typeof boundaryMeta?.leftContext === "string" && boundaryMeta.leftContext.length > 0) ||
        (typeof boundaryMeta?.rightContext === "string" && boundaryMeta.rightContext.length > 0);
      const pairDiagnostics =
        pairBeforeAnchor && pairAfterAnchor
          ? resolveTokenPairMatchInText(
              liveText,
              sourceTextForPair,
              suggestion,
              pairBeforeAnchor,
              pairAfterAnchor,
              {
                beforeHintIndex: anchor?.boundaryMeta?.sourceBoundaryStart,
                afterHintIndex: anchor?.boundaryMeta?.sourceBoundaryEnd,
                preferredGapHintIndex:
                  Number.isFinite(expectedBoundaryLiveHint) && expectedBoundaryLiveHint >= 0
                    ? expectedBoundaryLiveHint
                    : null,
                beforeOccurrence: Number.isFinite(anchor?.boundaryMeta?.beforeTokenOccurrence)
                  ? anchor.boundaryMeta.beforeTokenOccurrence
                  : null,
                afterOccurrence: Number.isFinite(anchor?.boundaryMeta?.afterTokenOccurrence)
                  ? anchor.boundaryMeta.afterTokenOccurrence
                  : null,
                allowQuoteGap: true,
                allowCommaInGap: true,
                maxGapLength: suggestion?.meta?.lemmaAnchorAuthoritative ? 18 : 10,
          ...
          : {
              match: null,
              ambiguous: false,
              reason: "missing_pair_anchors",
              candidateCount: 0,
              topCandidateCount: 0,
                };
      const pairBoundaryPos = Number.isFinite(pairDiagnostics?.match?.beforeEnd)
        ? pairDiagnostics.match.beforeEnd
        : Number.isFinite(pairDiagnostics?.match?.afterStart)
          ? pairDiagnostics.match.afterStart
          : -1;
      const opMeta = suggestion?.meta?.op || {};
      const opPrimarySourceHint = firstFiniteValue([
        opMeta?.correctedPos,
        opMeta?.pos,
        opMeta?.originalPos,
      ]);
      const opPrimaryLiveHint = Number.isFinite(opPrimarySourceHint)
        ? sourceTextForPair.length
          ? mapIndexAcrossCanonical(sourceTextForPair, liveText, opPrimarySourceHint, {
              allowEnd: true,
            })
          : Math.max(0, Math.min(liveText.length, Math.floor(opPrimarySourceHint)))
        : -1;
      const maxOpBoundaryDrift = suggestion?.meta?.lemmaAnchorAuthoritative ? 4 : 3;
      const boundaryNearPrimaryOpHint =
        !Number.isFinite(opPrimaryLiveHint) ||
        opPrimaryLiveHint < 0 ||
        (Number.isFinite(pairBoundaryPos) &&
          pairBoundaryPos >= 0 &&
          Math.abs(pairBoundaryPos - opPrimaryLiveHint) <= maxOpBoundaryDrift);
      const lexicalBoundaryAuthoritative =
        Boolean(opMeta?.fromApiCommaOps) ||
        Boolean(opMeta?.fromCorrections && !opMeta?.viaDiffFallback && !opMeta?.fromSyntheticApiOps);
      const fingerprintDiagnostics = hasBoundaryFingerprintHints
        ? evaluateBoundaryFingerprintUniquenessInText(
            liveText,
            boundaryMeta,
            Number.isFinite(expectedBoundaryLiveHint) && expectedBoundaryLiveHint >= 0
              ? expectedBoundaryLiveHint
        ...
        : null;
      const fingerprintBoundaryPos = Number.isFinite(fingerprintDiagnostics?.resolvedPos)
        ? fingerprintDiagnostics.resolvedPos
        : -1;
      const maxPairBoundaryDrift = suggestion?.meta?.lemmaAnchorAuthoritative ? 10 : 6;
      const boundaryNearExpectedHint =
        !Number.isFinite(expectedBoundaryLiveHint) ||
        expectedBoundaryLiveHint < 0 ||
        (Number.isFinite(pairBoundaryPos) &&
          pairBoundaryPos >= 0 &&
          Math.abs(pairBoundaryPos - expectedBoundaryLiveHint) <= maxPairBoundaryDrift);
      const fingerprintNearExpectedHint =
        !Number.isFinite(expectedBoundaryLiveHint) ||
        expectedBoundaryLiveHint < 0 ||
        (Number.isFinite(fingerprintBoundaryPos) &&
          fingerprintBoundaryPos >= 0 &&
          Math.abs(fingerprintBoundaryPos - expectedBoundaryLiveHint) <= maxPairBoundaryDrift);
      const allowTrustedFallbackBeforeGap =
        !pairDiagnostics?.ambiguous &&
        Number.isFinite(pairDiagnostics?.topCandidateCount) &&
        pairDiagnostics.topCandidateCount <= 1;
      const allowMissingFingerprintForTrustedPair =
        hasBoundaryFingerprintHints &&
        fingerprintDiagnostics?.reason === "fingerprint_not_found" &&
        allowTrustedFallbackBeforeGap &&
        Number.isFinite(pairBoundaryPos) &&
        pairBoundaryPos >= 0 &&
        boundaryNearExpectedHint;
      const fingerprintValid = !hasBoundaryFingerprintHints
        ? true
        : (Boolean(fingerprintDiagnostics?.unique) && fingerprintNearExpectedHint) ||
          allowMissingFingerprintForTrustedPair;
      const maxTrustedPairOpBoundaryDrift = suggestion?.meta?.lemmaAnchorAuthoritative ? 8 : 7;
      const boundaryNearPrimaryOpHintEffective =
        boundaryNearPrimaryOpHint ||
        (allowTrustedFallbackBeforeGap &&
          boundaryNearExpectedHint &&
          fingerprintValid &&
          Number.isFinite(opPrimaryLiveHint) &&
          ...
          Number.isFinite(pairBoundaryPos) &&
          pairBoundaryPos >= 0 &&
          Math.abs(pairBoundaryPos - opPrimaryLiveHint) <= maxTrustedPairOpBoundaryDrift);
      const beforeGapPathAllowed =
        !shouldPreferBoundaryBeforeLexicalAnchor ||
        lexicalBoundaryAuthoritative ||
        allowTrustedFallbackBeforeGap;
      if (
        !pairDiagnostics?.match ||
        !boundaryNearExpectedHint ||
        !boundaryNearPrimaryOpHintEffective ||
        !fingerprintValid ||
        !beforeGapPathAllowed
      ) {
        onlineAmbiguousInsertAnchorSkips++;
        const skipReason = !pairDiagnostics?.match
          ? pairDiagnostics?.reason || "ambiguous_anchor_pair"
          : !boundaryNearExpectedHint
            ? "pair_boundary_drift"
            : !boundaryNearPrimaryOpHintEffective
              ? "pair_op_hint_drift"
              : !beforeGapPathAllowed
                ? "non_authoritative_before_gap_path"
            : fingerprintDiagnostics?.reason || "boundary_fingerprint_mismatch";
        warn("highlight insert skipped: ambiguous anchor pair", {
          suggestionId: suggestion?.id ?? null,
          paragraphIndex: suggestion?.paragraphIndex ?? null,
          reason: skipReason,
          expectedBoundaryHint: Number.isFinite(expectedBoundaryLiveHint) ? expectedBoundaryLiveHint : null,
          resolvedBoundary: Number.isFinite(pairBoundaryPos) ? pairBoundaryPos : null,
          fingerprintBoundary: Number.isFinite(fingerprintBoundaryPos) ? fingerprintBoundaryPos : null,
          beforeToken: pairBeforeAnchor?.tokenText || null,
          afterToken: pairAfterAnchor?.tokenText || null,
          ambiguous: Boolean(pairDiagnostics?.ambiguous),
          candidateCount: Number.isFinite(pairDiagnostics?.candidateCount)
            ? pairDiagnostics.candidateCount
            : 0,
          topCandidateCount: Number.isFinite(pairDiagnostics?.topCandidateCount)
            ? pairDiagnostics.topCandidateCount
            : 0,
          primaryOpHint: Number.isFinite(opPrimaryLiveHint) ? opPrimaryLiveHint : null,
          lexicalBoundaryAuthoritative,
          hasBoundaryFingerprintHints,
          fingerprintUnique: Boolean(fingerprintDiagnostics?.unique),
          fingerprintAmbiguous: Boolean(fingerprintDiagnostics?.ambiguous),
          fingerprintPositions: Array.isArray(fingerprintDiagnostics?.positions)
            ? fingerprintDiagnostics.positions.length
            : 0,
        });
        return false;
      }
...
async function tryApplyInsertUsingMetadata(context, paragraph, suggestion) {
  ...
  const replaceGapBetweenAnchors = async (beforeAnchor, afterAnchor, traceLabel) => {
    ...
    const gapText = liveText.slice(beforeEnd, afterStart);
    if (hasBlockingBoundaryCommaInGap(gapText)) return true;
...
function hasBlockingBoundaryCommaInGap(gapText) {
  if (typeof gapText !== "string" || !gapText.length) return false;
  const commaIndex = gapText.indexOf(",");
  if (commaIndex < 0) return false;
  const prefix = gapText.slice(0, commaIndex);
  for (const char of prefix) {
    if (WORD_CHAR_REGEX.test(char || "")) {
      // A word exists before the first comma inside the gap (e.g. " mislim, "),
      // so this is not an already-present boundary comma after the before-token.
      return false;
    }
  }
  return true;
          ...
          if (deferredSuggestions.length) {
            ...
            for (let deferredIndex = 0; deferredIndex < deferredApplyPlan.length; deferredIndex++) {
              ...
              if (deferredOp?.kind === "insert" && deferredOpSuggestions.length) {
                ...
                if (
                  !hasAnchoredTokenPair &&
                  typeof deferredOp?.replacement === "string" &&
                  deferredOp.replacement.includes(",")
                ) {
...
async function checkDocumentTextOnline(checkToken) {
  ...
  onlineAmbiguousInsertAnchorSkips = 0;
  let paragraphsProcessed = 0;
  let suggestionsDetected = 0;
  let suggestions = 0;
  let apiErrors = 0;
