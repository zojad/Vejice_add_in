...
export class CommaSuggestionEngine {
  ...
  async analyzeParagraph({
    ...
    const correctedChunkOffsetByKey = new Map();
    let correctedCursor = 0;
    for (const meta of processedMeta) {
      if (!meta?.chunk) continue;
      correctedChunkOffsetByKey.set(chunkMetaKey(meta.chunk), correctedCursor);
      const correctedTextPart = typeof meta.correctedText === "string" ? meta.correctedText : "";
      const trailingTextPart = typeof meta.chunk.trailing === "string" ? meta.chunk.trailing : "";
      correctedCursor += correctedTextPart.length + trailingTextPart.length;
    }
    ...
    for (const entry of chunkDetails) {
      ...
      for (let opIndex = 0; opIndex < allOps.length; opIndex++) {
        ...
        const originalOffset = Number.isFinite(entry?.chunk?.start) ? entry.chunk.start : 0;
        const correctedOffset =
          correctedChunkOffsetByKey.get(chunkMetaKey(entry?.chunk)) ?? originalOffset;
        const baseOp = op;
        const adjustedOp = {
          ...baseOp,
          pos: baseOp.pos + originalOffset,
          originalPos:
            (typeof baseOp.originalPos === "number" ? baseOp.originalPos : baseOp.pos) +
            originalOffset,
          correctedPos:
            (typeof baseOp.correctedPos === "number" ? baseOp.correctedPos : baseOp.pos) +
            correctedOffset,
